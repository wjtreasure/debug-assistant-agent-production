...
class MockLLMClient(LLMClient):
    ...
    def complete_json(self, system: str, user: str, *, model: str | None=None) -> dict[str, Any]:
        self.n+=1; self.calls.append({"model":"mock","input_tokens":0,"output_tokens":0,"total_tokens":0})
        low=user.lower()
        ...
        if 'reflection_schema' in low:
            return {"decision":"continue","reason":"collect one more targeted source excerpt","missing":["target implementation"],"contradictions":[]}
        if self.n == 1:
        ...
        return {"kind":"finish","skill":"report_synthesis","reason":"enough evidence for smoke test","confidence":0.75,"arguments":{},"expected_evidence":""}
