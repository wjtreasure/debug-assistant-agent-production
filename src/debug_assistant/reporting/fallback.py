...
class FallbackReportBuilder:
    ...
    def build(self, task_id: str, hypothesis, evidence) -> DiagnosisReport:
        ...
        change_points=build_runtime_change_points(hypothesis, selected)
        files=[]; symbols=[]
        for e in selected:
        ...
        return DiagnosisReport(
            task_id=task_id,summary=desc,root_cause=desc,likely_files=files[:3],likely_symbols=symbols,
            impact_scope=[],evidence=self._ev_rows(selected),recommended_change_points=change_points,uncertainties=list(hypothesis.missing_evidence or []),
    ...
    def build_from_evidence(self, task_id:str, evidence, candidate_files:list[str]|None=None, hypothesis=None) -> DiagnosisReport:
        selected=[e for e in evidence if e.source=='read_file'] or list(evidence)
        files=list(candidate_files or [])
        for e in selected:
            if e.file and e.file not in files: files.append(e.file)
        change_points=build_runtime_change_points(hypothesis, selected) if hypothesis is not None else []
        for point in reversed(change_points):
            if point.get('file') in files:
                files.remove(point['file'])
            files.insert(0, point['file'])
        ...
        return DiagnosisReport(
            ...
            impact_scope=[],evidence=self._ev_rows(selected),recommended_change_points=change_points,
